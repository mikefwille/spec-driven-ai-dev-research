...
if [ -n "$session_id" ] && [ "$HAS_JQ" -eq 1 ]; then
  ...
  # Use the transcript_path provided by Claude Code directly
  ...
  if [ -f "$session_file" ]; then
    ...
    if [ -n "$latest_tokens" ] && [ "$latest_tokens" -gt 0 ]; then
      ...
      # Set color based on used percentage (red at 40% or more)
      if [ "$context_used_pct" -ge 40 ]; then
...
# ---- AI conversation summary (using Claude Haiku) ----
summary_line=""
if [ -f "$transcript_path" ] && command -v claude >/dev/null 2>&1; then
  # Extract recent conversation text (last 10 text messages)
  # User messages have .message.content as string, assistant messages as array
  conversation=$(tail -50 "$transcript_path" 2>/dev/null | \
  ...
  if [ -n "$conversation" ]; then
    # Call claude with Haiku to generate a short summary
    summary=$(echo "$conversation" | \
      claude -p "tell me exactly what I am sharing with you. repeat it verbatim :" \
      --model haiku \
      2>/dev/null | \
      head -1)

    if [ -n "$summary" ]; then
      summary_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;183m'; fi; }  # light purple
      summary_line="💭 $(summary_color)${summary}$(rst)"
    fi
  fi
fi
